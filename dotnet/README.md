...
## Описание 
> [!NOTE] К сведению  
> Версия NuGet пакета `AWS.SDK` в примере самая актуальная на Июнь 2026 (`<=4.0.100`)
...
В качестве клиента к S3 хранилищем Timeweb Cloud используется официальный SDK от Amazon (`AWS.SDK`) версии `4.0.100`, ендпоинт Timeweb Cloud S3 API  `https://s3.twcstorage.ru`  
